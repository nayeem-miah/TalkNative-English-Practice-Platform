...
export function CourseDetailsClient({ initialCourse, courseId }: CourseDetailsClientProps) {
  const { data: courseResponse, isLoading: isQueryLoading, refetch } = useGetCourseByIdQuery(courseId)
  ...
  React.useEffect(() => {
    if (mounted && course?.id) {
      try {
        ...
        if (saved) {
          completedLessonIds.length === 0 && setCompletedLessonIds(JSON.parse(saved))
        }
      } catch (e) {
        console.error(e)
      }
            ...
            <div className="space-y-3 max-w-3xl">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-muted-foreground font-semibold">
                ...
                {course.averageRating > 0 ? (
                  ...
                  <>
                    <span>•</span>
                    <span className="text-muted-foreground/60 text-xs font-semibold">No ratings yet</span>
                  </>
                )}
                {course.isEnrolled && (
                  <>
